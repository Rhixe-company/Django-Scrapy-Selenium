    ...
    <div className="lg:my-0 relative max-[786px]:p-0 max-[882px]:p-4 min[925px]:p-0">
      <div className="flex flex-col items-center space-y-2 pt-6 px-5 text-center">
        ...
        <p className="text-sm text-[#999999]">
          All chapters are in
          <Link href={`/comic/${chapter.comic.slug}`}>
        ...
        <div className="flex items-center justify-center gap-1 py-2">
          <Link
            href={`https://www.facebook.com/sharer/sharer.php?u=/comic/${chapter.comic.slug}/chapter/${chapter.slug}&amp;t=${chapter.comic.title}`}
            target="_blank"
          ...
          <Link
            href={`https://www.twitter.com/intent/tweet?url=/comic/${chapter.comic.slug}/chapter/${chapter.slug}&amp;text=${chapter.comic.title}`}
            target="_blank"
          ...
          <Link
            href={`whatsapp://send/?text=${chapter.comic.title}/comic/${chapter.comic.slug}/chapter/${chapter.slug}`}
            target="_blank"
          ...
          <Link
            href={`https://pinterest.com/pin/create/button/?url=/comic/${chapter.comic.slug}/chapter/${chapter.slug}&amp;media=${chapter.comic.images[0].link}&amp;description=${chapter.comic.title}`}
            target="_blank"
        ...
        <h2 className="text-center md:text-center py-4 text-sm text-[#999999]">
          ...
          is always updated at <span className="font-bold">Rhixe Scans</span>.
          Dont forget to read the other manga updates. A list of manga
          collections
          <span className="font-bold pl-1">Rhixe Scans</span> is in the Manga
          List menu.
        </h2>
        <div className="flex flex-row flex-wrap gap-y-2 justify-between items-center md:justify-between w-full">
          ...
          <div className="flex items-center gap-x-3 flex-row w-full sm:w-40 justify-between sm:self-end">
            <Link href={`/comic/${chapter.comic.slug}/chapter/${chapter.slug}`}>
        ...
        <div className="py-8 -mx-5 md:mx-0 flex flex-col items-center justify-center">
          ...
          {chapter.images.map((myimg, index: React.Key | null | undefined) => (
            <div key={index}>
        ...
        <div className="space-y-4">
          <div className="bg-[#222222] px-5 py-4 flex items-center md:text-start justify-center gap-x-2 w-full">
            <h2 className="text-[13px] text-[#999999]">
              tags: read manga {chapter.comic.title} {chapter.name}, comic
              {chapter.comic.title}
              {chapter.name}, read{chapter.comic.title} {chapter.name} online,
              {chapter.comic.title}
              <span className="px-1">{chapter.name}</span>
              chapter, {chapter.comic.title} {chapter.name}chapter,
              {chapter.comic.title} {chapter.name}
              high quality, {chapter.comic.title} {chapter.name} manga scan,
              {chapter.updated_at},
            </h2>
          </div>
          <div className="bg-[#222222]">
            ...
            <div className="grid md:hidden grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 md:grid-cols-5 xl:grid-cols-7 gap-3 bg-[#222222] p-4">
              {chapter.comic.related_series.map((item) => (
...
export async function generateStaticParams() {
  ...
  if (chapters?.total_results === 0) return undefined;
