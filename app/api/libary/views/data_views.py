...
def data_table_view(request, **kwargs):
    try:
        model_class = Utils.get_class(DYNAMIC_DATATB, kwargs.get("model_name"))  # type: ignore
    except KeyError:
    ...
    return render(
        ...
        context={
            ...
            "is_date": [
                True if type(field) == DateField else False
                for field in model_class._meta.get_fields()
            ],
            "total_pages": range(1, math.ceil(all_data.count() / entries) + 1),
            "has_prev": False
            if page_number == 1
            else (True if all_data.count() != 0 else False),
            "has_next": False
...
def add_record(request, **kwargs):
    ...
    try:
        ...
        return HttpResponse(
            json.dumps({"detail": str(ve), "success": False}), status=400
        )
    return HttpResponse(
...
def delete_record(request, **kwargs):
    ...
    return HttpResponse(
        json.dumps({"message": "Record Deleted.", "success": True}), status=200
    )
...
def edit_record(request, **kwargs):
    ...
    return HttpResponse(
        json.dumps({"message": "Record Updated.", "success": True}), status=200
    )
...
def export(request, **kwargs):
    ...
    df = pd.DataFrame(table_data, columns=tuple(heading for heading in headings))
    if export_type == "pdf":
