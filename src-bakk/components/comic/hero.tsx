import Rightbar from "../base/Rightbar";

import type { Comic } from "../../types/ComicType";
import Image from "next/image";
import Link from "next/link";

export default function hero({ item }: { item: Comic }) {
  return (
    <>
          ...
          <div className="space-y-7">
                ...
                {item.images.length > 1 && (
                  <div className="bigcover">
                    <Image
                      ...
                      src={`http://localhost:8000${item.images[1].image}`}
                    />
                  </div>
                )}
                ...
                >
                  ...
                  <div className="col-span-12 sm:col-span-9 pt-10">
                    ...
                    <div className="flex items-center justify-center sm:justify-start flex-wrap gap-1 py-4">
                      ...
                      <Link
                        href={`whatsapp://send/?text=${item.title}/series/${item.title}/`}
                        target="_blank"
                      ...
                      <Link
                        href={`https://pinterest.com/pin/create/button/?url=/series/${item.title}&amp;media=http://localhost:8000${item.images[0].image}&amp;description=${item.title}`}
                        target="_blank"
            ...
            <div className="bg-[#222222]">
              ...
              <div className="grid grid-cols-2 px-4 py-4 gap-2.5">
                <Link
                  href={`/series/${item.slug}/chapter/${item.first_chapter.slug}`}
                >
                  <div className="flex flex-col items-center justify-center py-4 rounded-sm bg-themecolor hover:bg-[#343434] transition-all duration-300 cursor-pointer text-white">
                    ...
                    <h3 className="font-bold text-xl">
                      {item.first_chapter.name}
                    </h3>
                  </div>
                </Link>
                <Link
                  href={`/series/${item.slug}/chapter/${item.last_chapter.slug}`}
                >
                  <div className="flex flex-col items-center justify-center py-4 rounded-sm bg-themecolor hover:bg-[#343434] transition-all duration-300 cursor-pointer text-white">
              ...
              <div>
                <div className="px-4 pb-4">
                  <input
                    ...
                    placeholder={`Search Chapter. Example: ${item.first_chapter.name} or ${item.last_chapter.name}`}
                    type="text"
                  />
                </div>
                <div className="pl-4 pr-2 pb-4 overflow-y-auto scrollbar-thumb-themecolor scrollbar-track-transparent scrollbar-thin mr-3 max-h-[20rem] space-y-2.5">
                  {item.chapters &&
                    item.chapters.map((chapter) => (
                      ...
                      >
                        <Link
                          href={`/series/${item.slug}/chapter/${chapter.slug}`}
                        >
